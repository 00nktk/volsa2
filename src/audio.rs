...
use hound::{
	Result as WavResult, SampleFormat, WavReader, WavSpec, WavWriter,
};
...
impl AudioReader<'_, ()> {
	...
	pub fn open_file(
		path: &Path,
	) -> Result<AudioReader<'_, impl Iterator<Item = AudioItem>>> {
	...
	I: Iterator<Item = AudioItem>,
{
	pub fn take_channel(
		self,
		channel: u8,
	) -> AudioReader<'a, impl Iterator<Item = AudioItem>> {
		...
		let reader = self
			...
			.enumerate()
			.filter(move |(idx, _)| {
				idx % channels as usize == channel as usize
			})
			.map(|(_, sample)| sample);
...
#[auto_enum]
fn into_samples_f64<R>(
	reader: WavReader<R>,
) -> Result<impl Iterator<Item = WavResult<f64>>>
where
	R: io::Read,
{
	let spec = reader.spec();
	...
	let iter = match (spec.sample_format, spec.bits_per_sample) {
		...
		(SampleFormat::Float, 32) => {
			reader.into_samples::<f32>().map(|res| res.map(Into::into))
		}
