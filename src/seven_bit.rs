...
mod tests {
        ...
        F::Input: Debug + PartialEq + Clone,
    {
        ...
        let converted_data = Converter::<_, F>::new(data.iter().cloned()).collect::<Vec<_>>();
        assert_eq!(
            converted_data.len(),
            predicted_size,
        ...
        let recovered_data = Converter::<_, B>::new(converted_data.into_iter()).collect::<Vec<_>>();
        ...
        C::OutputBuffer: Zeroable,
    {
        ...
        for chunk in data.chunks(C::InputBuffer::LEN) {
            ...
            converted_data_expected.extend(chunk.into_iter().take(to_take as usize));
        }

        let converted_data = Converter::<_, C>::new(data.into_iter()).collect::<Vec<_>>();
