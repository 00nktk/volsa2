...
use super::{
	Incoming, Message, Outgoing, ParseError, Version, VOLCA_SAMPLE_2_ID,
};
...
impl Incoming for Status {
	fn parse_data(slice: &[u8]) -> Result<Self, ParseError> {
		let (status, _) =
			slice.split_first().ok_or(ParseError::NotEnoughData)?;
		let status = match *status {
			ACK_STATUS => Ok(()),
			x if x == NakStatus::Busy as u8 => Err(NakStatus::Busy),
			x if x == NakStatus::SampleFull as u8 => {
				Err(NakStatus::SampleFull)
			}
			x if x == NakStatus::DataFormat as u8 => {
...
impl Incoming for SearchDeviceReply {
	fn parse_data(slice: &[u8]) -> Result<Self, ParseError> {
		...
		let (channel, echo, model_id, minor, major) =
			array_refs![slice, 1, 1, 4, 2, 2];
		...
		let version =
			Version(u16::from_le_bytes(*major), u16::from_le_bytes(*minor));

		Ok(Self {
			device_id: U7::new_checked(channel[0])
