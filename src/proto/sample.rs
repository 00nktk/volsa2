...
impl Incoming for SampleSpaceDump {
	fn parse_data(slice: &[u8]) -> Result<Self, ParseError> {
		...
		let (&[used_lsb, used_msb], &[all_lsb, all_msb]) =
			array_refs![slice, 2, 2];
...
impl Incoming for SampleHeader {
	...
	fn parse_data(slice: &[u8]) -> Result<Self, ParseError> {
		...
		let mut data: Vec<u8> =
			FromKorgData::new(data.iter().copied().map(U7::new)).collect();
		if data.len() < 32 {
		...
		let (length, level, speed) =
			array_type_refs![sample_props, u32, u16, u16];
...
impl Incoming for SampleData {
	fn parse_data(slice: &[u8]) -> Result<Self, ParseError> {
		let (sample_no, data) = read_u8(slice);
		let mut buf =
			Vec::with_capacity(U7ToU8::convert_len(data.len()) / 2 + 1);
...
mod tests {
	...
	fn test_template(idx: usize) {
		let expected =
			WavReader::open(format!("test_data/sample{idx}.wav.raw"))
				.unwrap()
				.into_samples::<i16>()
		...
		let data_dump =
			File::open(format!("test_data/sample_data_dump{idx}.raw"))
				.unwrap()
				.bytes()
				.collect::<Result<Vec<_>, _>>()
				.unwrap();
