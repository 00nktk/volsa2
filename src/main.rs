...
impl App {
	...
	fn list_samples(&mut self, show_empty: bool) -> Result<()> {
		...
		for header in volca.iter_sample_headers().filter(|res| {
			res.as_ref().map_or(true, |header| !header.is_empty())
		}) {
			let header = header?;
			if show_empty {
		...
		let sample_no = sample_no
			...
			.or_else(|| {
				volca.iter_sample_headers().find_map(|result| {
					result
						.map(|header| {
							header.is_empty().then_some(header.sample_no)
						})
						.transpose()
				})
			})
	...
	fn load_audio_file(path: &Path, mono_mode: MonoMode) -> Result<Vec<i16>> {
		...
		let sample = match (reader.channels(), mono_mode) {
			(1, _) | (_, MonoMode::Left) => {
				reader.take_channel(0).resample_to_volca()?
			}
			(_, MonoMode::Right) => {
				reader.take_channel(1).resample_to_volca()?
			}
...
fn main() -> Result<()> {
	...
	match opts.cmd {
		...
		opt::Operation::Upload {
			...
			file, sample_no, mono_mode, output, dry_run, None, true,
		)?,
